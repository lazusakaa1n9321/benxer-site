...
$title =  "En Çok İzlenenler - ".$genel_ayarlar['title']." | Video İzle | Video İndir | Video Seyret | Müzik Dinle | Müzik İndir";
...
<div id="icerik">
  <!-- İçerik-Sol Başlar -->
  <div id="sol">
			<div class="sol_baslik">En Çok İzlenen Videolar</div>
        <div class="sol_kutu">
        	<?php
			...
			$video = mysql_query("SELECT * FROM lynvideo_video WHERE durum='1' ORDER BY hit DESC limit $alt,$max");	
		   ...
		   if ($videosayi >= $max){
			...
			$x = $perpage;
			  ...
			  else echo '<a href="en-cok-izlenenler/sayfa/1">1</a>';  
			  if($page-$x > 2) {
				echo "...";
				$i = $page-$x;
			  } else {
			  ...
			  for($i; $i<=$page+$x; $i++) {
				...
				else echo '<a href="en-cok-izlenenler/sayfa/'.$i.'">'.$i.'</a>';
				if($i==$lastP) break;
			  }
			  if($page+$x < $lastP-1) {
				echo "...";
				echo '<a href="en-cok-izlenenler/sayfa/'.$lastP.'">'.$lastP.'</a>';
